...
def read_OPTA_f24(fpath, fname, match_opta):
    ...
    data = f.read()
    f.close()
    # deal with xml markup (BS might be overkill here)
    soup = BeautifulSoup(data, 'xml')
    game = soup.find('Game')
    match_opta.add_gamedata(game, match_opta, typeids, qualids)
    match_opta.hometeam.get_team_events(
        match_opta.events, match_opta.awayteam.team_id, match_opta)
    match_opta.awayteam.get_team_events(
        match_opta.events, match_opta.hometeam.team_id, match_opta)
    return match_opta
...
class OPTAmatch(object):
    ...
    def __init__(self, matchdata):
        ...
        self.total_match_time = int(matchdata.find(
            'Stat', {"Type": "match_time"}).text)
    ...
    def add_gamedata(self, gamedata, match, typeids, qualids):
        ...
        assert utils.remove_accents(
            gamedata['home_team_name']) == self.hometeam.teamname
        assert utils.remove_accents(
            gamedata['away_team_name']) == self.awayteam.teamname
        self.season = gamedata['season_id']
        date_check = dt.datetime.strptime(
            gamedata['game_date'][:10], '%Y-%m-%d')
    ...
    def __repr__(self):
        s = "%s vs %s on %s" % (self.hometeam.teamname,
                                self.awayteam.teamname, self.datestring)
        return s
...
class OPTAevent(object):
    ...
    def __init__(self, event, match_OPTA, typeids, qualids):
        ...
        # put the origin at the center of the pitch
        self.x = (float(event['x'])/100.-0.5)
        # put the origin at the center of the pitch
        self.y = (float(event['y'])/100.-0.5)
    ...
    def get_assist(self, events, match_OPTA):
        ...
        if 55 in self.qual_id_list:
            self.assisted = True
            assist_event_id = int(
                self.qualifiers[self.qual_id_list.index(55)].value)
            related_events = [
                e for e in events if e.event_id == assist_event_id]
        ...
        if 216 in self.qual_id_list:
            ...
            assist_event_id = int(
                self.qualifiers[self.qual_id_list.index(216)].value)
            related_events = [
                e for e in events if e.event_id == assist_event_id]
            if len(related_events) == 0:
                print("no related event found")
    ...
    def find_opponent_error(self, opponent_events, t_window=6):
        ...
        opp_events_window = [
            e for e in opponent_events if self.time-e.time < t_window/60.]
        self.opponent_error = [
            e for e in opp_events_window if 69 in e.qual_id_list]
    ...
    def calc_shot_xG_inputs(self, match_OPTA):
        ...
        # x-distance to opponents goal in m
        self.xgoal = (0.5-self.x)*match_OPTA.fPitchXSizeMeters
        # y-distance to center of opponents goal in m
        self.ygoal = self.y*match_OPTA.fPitchYSizeMeters
        # distance to center of goal in m
        self.distance = np.sqrt(self.xgoal**2+self.ygoal**2)
        self.angle_to_centre = np.arccos(self.xgoal/self.distance)*180./np.pi
        # x distance to posts (nearest,furthest)
        x_to_posts = (np.abs(np.abs(self.ygoal)-posts),
                      np.abs(self.ygoal)+posts)
    ...
    def calc_shot_xG_Caley(self, xG_pen=0.75):
        ...
        if np.abs(self.ygoal) <= central_channel_width:
            ...
            relative_angle_to_goal = np.arctan(
                self.xgoal/np.abs(self.ygoal))/np.arctan(self.xgoal/central_channel_width)
        ...
        if self.penalty:
            ...
            if self.assist_cross:
                ...
                xG = 1.02 * np.exp(-0.12 * adj_distance_to_goal) + \
                    0.21 * self.assist_throughball + 0.017
                self.caley_types = 5
            else:
                # Regular shots
                xG = 0.86 * np.exp(-0.14 * adj_distance_to_goal) + 0.15 * \
                    self.assist_throughball + 0.045 * self.direct_free_kick + 0.012
                self.caley_types = 6
        self.expG_caley = xG
    ...
    def calc_shot_xG_Caley2(self, xG_pen=0.75):
        ...
        if np.abs(self.ygoal) <= central_channel_width:
            ...
            relative_angle_to_goal = np.arctan(
                self.xgoal / (np.abs(self.ygoal)-central_channel_width)) / (np.pi/2.)
        # * self.meters_to_yards
        distance_to_goal = np.sqrt(self.xgoal**2+self.ygoal**2)
        ...
        if self.penalty:
            ...
            x = -3.84 + -0.08*distance_to_goal + 98.7*inv_distance_to_goal + \
                3.54*inv_rel_angle_to_goal - 91.1*inv_angle_distance_prod
            self.caley_types2 = 1
        elif self.header or self.other_body_part:
            if self.assist_cross:
                # Headers off crosses
                x = -2.88 + -0.21*distance_to_goal + 2.13*relative_angle_to_goal + 4.31 * \
                    self.assist_inv_distance_to_goal + 0.46*self.assist_relative_angle_to_goal
                x += 0.2*self.fast_break + -0.24*self.from_corner + 1.2*self.big_chance + \
                    1.1*self.shot_following_error + -0.18*self.other_body_part
                self.caley_types2 = 2
            else:
                # Headers, not off crosses
                x = -3.85 + -0.1*distance_to_goal + 2.56*inv_distance_to_goal + \
                    1.94*relative_angle_to_goal + 0.51*self.assist_throughball
                x += 0.44*self.fast_break + 1.3*self.big_chance + 1.1 * \
                    self.shot_following_error + 0.7*self.shot_off_rebound + 1.14*self.other_body_part
                self.caley_types2 = 3
        else:  # shots with feet
            if self.assist_cross:
                # Non-headed shots off crosses
                x = -2.8 + -0.11*distance_to_goal + 3.52*inv_distance_to_goal + 1.14*relative_angle_to_goal + \
                    6.94*self.assist_inv_distance_to_goal + 0.59*self.assist_relative_angle_to_goal
                x += 0.24*self.fast_break + -0.12*self.from_corner + \
                    1.25*self.big_chance + 1.1*self.shot_following_error
                self.caley_types2 = 4
            else:
                # Regular shots
                x = -3.190 + -0.095*distance_to_goal + 3.180*inv_distance_to_goal + 1.880 * \
                    relative_angle_to_goal + 0.240*inv_rel_angle_to_goal + -2.090*inv_angle_distance_prod
                x += 0.450*self.assist_throughball + 0.640*self.second_assist_throughball + \
                    2.180*self.assist_inv_distance_to_goal + \
                    0.120*self.assist_relative_angle_to_goal
                x += 0.23*self.fast_break + -0.18*self.from_corner + 1.2*self.big_chance + 1.1 * \
                    self.shot_following_error + 0.390 * \
                    self.individual_play + 0.370*self.shot_off_rebound
                self.caley_types2 = 5
    ...
    def calc_caley2_dist_angle(self, event, match_OPTA):
        ...
        # x-distance to opponents goal in m
        xgoal = (0.5-event.x)*match_OPTA.fPitchXSizeMeters
        # y-distance to center of opponents goal in m
        ygoal = event.y*match_OPTA.fPitchYSizeMeters
        ...
        if np.abs(ygoal) <= central_channel_width:
            ...
            relative_angle_to_goal = np.arctan(
                xgoal / (np.abs(ygoal)-central_channel_width)) / (np.pi/2.)
    ...
    def __repr__(self):
        if self.is_shot:
            s = "%s, %1.2f, %s, xG: %1.2f" % (
                self.team_name, self.time, self.description, self.expG_caley)
        else:
