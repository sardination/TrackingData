...
import Tracking_Structure as struc
...
for match in match_ids:
    ...
    A,D = struc.calc_formation_by_period(60*2,60*1,posH,frames_tb,match_tb,10,team1_exclude,team0_exclude,plotfig=False,deleteLattices=True)
    A2,D2 = struc.calc_formation_by_period(60*2,60*1,posA,frames_tb,match_tb,10,team1_exclude,team0_exclude,plotfig=False,deleteLattices=True)
...
for i in range(0,4000):
    ...
    for j in range(i,len(all_formations)):
        if j<3900:
            ...
            cost,p1,pm,s = struc.Hungarian_Cost(all_formations[i],all_formations[j],match_tb,metric='WM',squeeze=[0.7,0.8,0.9,1.0,1.1,1.2,1.3,1.4,1.5],plot=False)
            Amatrix[i,j] = cost
            Smatrix[i,j] = s
...
fname = "formation_results_superliga10_2min_WM_cross.pkl"
