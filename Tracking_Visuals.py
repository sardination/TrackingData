...
def plot_frame(frame, match, alpha=1.0, include_player_velocities=True, include_ball_velocities=True, figax=None):
    ...
    for i, j in enumerate(frame.team0_jersey_nums_in_frame):
        ...
        team0[i, 2] = frame.team0_players[j].vx / \
            2.  # Divide by 2 for aesthetic purposes only (otherwise quiver is too long)
        team0[i, 3] = frame.team0_players[j].vy / \
            2.  # Divide by 2 for aesthetic purposes only (otherwise quiver is too long)
    team0 = team0[:i+1, :]
    pts1, = ax.plot(team1[:, 0], team1[:, 1], 'ro', linewidth=0,
                    markeredgewidth=0, markersize=10, alpha=alpha)
    pts2, = ax.plot(team0[:, 0], team0[:, 1], 'bo', linewidth=0,
                    markeredgewidth=0, markersize=10, alpha=alpha)
    ax.quiver(team1[:, 0], team1[:, 1], team1[:, 2], team1[:, 3],
              color='r', width=0.002, headlength=5, headwidth=3, scale=60)
    ax.quiver(team0[:, 0], team0[:, 1], team0[:, 2], team0[:, 3],
              color='b', width=0.002, headlength=5, headwidth=3, scale=60)
    if frame.ball:
        ax.plot(frame.ball_pos_x, frame.ball_pos_y, marker='o', markerfacecolor='k',
                markeredgewidth=0, markersize=6*max(1, np.sqrt(frame.ball_pos_z/150.)), alpha=alpha)
    ...
    ax.text(-150, match.fPitchYSizeMeters*50+40,
            frame.min+':' + frame.sec.zfill(2), fontsize=14)
    txt_offset = 30
...
def plot_frames(frames, match, pause=0.0, include_player_velocities=True, include_ball_velocities=False, skip=1, alpha=1.0):
    ...
    for frame in frames[::skip]:
        ...
        for i, j in enumerate(frame.team0_jersey_nums_in_frame):
            ...
            team0[i, 1] = frame.team0_players[j].pos_y
            team0[i, 2] = frame.team0_players[j].vx / \
                4.  # Divide by 2 for aesthetic purposes only (otherwise quiver is too long)
            team0[i, 3] = frame.team0_players[j].vy / \
                4.  # Divide by 2 for aesthetic purposes only (otherwise quiver is too long)
        team0 = team0[:i+1, :]
        pts1, = ax.plot(team1[:, 0], team1[:, 1], 'ro',
                        linewidth=0, markeredgewidth=0, markersize=10)
        pts2, = ax.plot(team0[:, 0], team0[:, 1], 'bo',
                        linewidth=0, markeredgewidth=0, markersize=10)
        pts6 = ax.quiver(team1[:, 0], team1[:, 1], team1[:, 2], team1[:, 3],
                         color='r', width=0.002, headlength=5, headwidth=3, scale=60)
        pts7 = ax.quiver(team0[:, 0], team0[:, 1], team0[:, 2], team0[:, 3],
                         color='b', width=0.002, headlength=5, headwidth=3, scale=60)
        if frame.ball and frame.ball_status == 'Alive':
            ball_on = True
            pts4, = ax.plot(frame.ball_pos_x, frame.ball_pos_y, marker='o', markerfacecolor='k',
                            markeredgewidth=0, markersize=6*max(1, np.sqrt(frame.ball_pos_z/150.)))
            if include_ball_velocities:
                x = np.array([0, frame.ball_vx*100]) + frame.ball_pos_x
                y = np.array([0, frame.ball_vy*100]) + frame.ball_pos_y
            ...
            ball_on = False
        pts3 = ax.text(-150, match.fPitchYSizeMeters*50+40,
                       frame.min+':' + frame.sec.zfill(2), fontsize=14)
        if pause > 0:
            plt.pause(pause)
        if not points_on:
...
def save_match_clip(frames, match, fpath, fname='clip_test', include_player_velocities=True, hcol='r', acol='b', team1_exclude=[], team0_exclude=[], include_hulls=False, speed_fact=1.0, description=None):
    ...
    metadata = dict(title='Tracking Data',
                    artist='Matplotlib', comment='test clip!')
    writer = FFMpegWriter(fps=match.iFrameRateFps *
                          speed_fact, metadata=metadata)
    fig, ax = plot_pitch(match)
    fig.suptitle(fname, fontsize=16, y=0.95)
    if description is not None:
        ax.text(-1*match.fPitchXSizeMeters/2. * 100, -1 *
                (match.fPitchYSizeMeters/2.+10)*100, description, fontsize=12, alpha=0.7)
    points_on = False
        ...
        for frame in frames:
            ...
            for i, j in enumerate(frame.team0_jersey_nums_in_frame):
                if j not in team0_exclude:
                    ...
                    c += 1
            team0 = team0[:c, :]
            pts1, = ax.plot(team1[:, 0], team1[:, 1], hcol+'o',
                            linewidth=0, markeredgewidth=0, markersize=10)
            pts2, = ax.plot(team0[:, 0], team0[:, 1], acol+'o',
                            linewidth=0, markeredgewidth=0, markersize=10)
            if include_player_velocities:
                pts6 = ax.quiver(team1[:, 0], team1[:, 1], team1[:, 2], team1[:, 3],
                                 color=hcol, width=0.002, headlength=5, headwidth=3, scale=80)
                pts7 = ax.quiver(team0[:, 0], team0[:, 1], team0[:, 2], team0[:, 3],
                                 color=acol, width=0.002, headlength=5, headwidth=3, scale=80)
            if include_hulls:
            ...
            if frame.ball:
                pts3, = ax.plot(frame.ball_pos_x, frame.ball_pos_y, marker='o', markerfacecolor='k',
                                markeredgewidth=0, markersize=6*max(1, np.sqrt(frame.ball_pos_z/150.)))
                if frame.ball_status != 'Alive' and not in_play_flag_on:
                    pts10 = ax.text(match.fPitchXSizeMeters*50-1500, match.fPitchYSizeMeters *
                                    50-250, 'Play stopped', fontsize=14, color='r')
                    in_play_flag_on = True
                elif frame.ball_status == 'Alive' and in_play_flag_on:
                    pts10.remove()
                    in_play_flag_on = False
            else:
                pts3, = ax.plot(0, 0, marker='x', markerfacecolor='k',
                                markeredgewidth=0, markersize=6)
            pts4 = ax.text(-150, match.fPitchYSizeMeters*50+40, str((frame.period-1)
                                                                    * 45+int(frame.min))+':' + frame.sec.zfill(2), fontsize=14)
            if not points_on:
                points_on = True
            writer.grab_frame()
    plt.close('all')
