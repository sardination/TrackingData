...
for match in match_ids[0:1]:
    ...
    # get all possessions
    posessions = tracab.get_tracab_posessions(
        frames_tb, match_tb, min_pos_length=1)
    ...
    posH = [p for p in posessions if p.team == 'H' and p.pos_type in ['A', 'N'] and (
        p.pos_duration > 5 or p.pos_start_type == 'DeadBall') and not p.bad_possesion]
    posA = [p for p in posessions if p.team == 'A' and p.pos_type in ['A', 'N'] and (
        p.pos_duration > 5 or p.pos_start_type == 'DeadBall') and not p.bad_possesion]
    ...
    A, D = form.calc_formation_by_period(max_pos_length, min_pos_length, posH, frames_tb,
                                         match_tb, subsample, team1_exclude, team0_exclude, plotfig=False, deleteLattices=True)
    A2, D2 = form.calc_formation_by_period(max_pos_length, min_pos_length, posA, frames_tb,
                                           match_tb, subsample, team1_exclude, team0_exclude, plotfig=False, deleteLattices=True)
...
for i in range(len(all_formations)):
    ...
    for j in range(i, len(all_formations)):
        if i == j:
            ...
            cost, p1, pm, s = form.Hungarian_Cost(all_formations[i], all_formations[j], match_tb, metric='WM', squeeze=[
                                                  0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3, 1.4, 1.5], plot=False)
            Amatrix[i, j] = cost
            Smatrix[i, j] = s
