...
# path to directory of Tracab data
fpath = '/PATH/TO/WHERE/YOU/SAVED/TRACKING/DATA/'

#fname_dict = tracab.get_filename_dict()
# filename of Tracab match
fname = 'SouthamptonManchester City2018-11-04T15,00,00Z.txt.g987698'
...
frames_tb, match_tb, team1_players, team0_players = tracab.read_tracab_match_data(
    'EPL', fpath, fname, verbose=True)  # frames is a list of the individual match snapshots (positions, velocities)
...
fig, ax = vis.plot_frame(
    frames_tb[1000], match_tb, include_player_velocities=True, include_ball_velocities=False)
...
for frame in frames_in_first_half:
    # sometimes the ball data isn't in the frame (for whatever reason)
    if frame.ball:
        ball_positions_xyz.append(
            [frame.ball_pos_x, frame.ball_pos_y, frame.ball_pos_z])
        ball_velocities_xyz.append(
            [frame.ball_vx, frame.ball_vy, frame.ball_vz])
...
flast = vis.find_framenum_at_timestamp(
    frames_tb, match_tb, 2, 0)  # first frame of second half
